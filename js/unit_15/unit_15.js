...
const f4 = () => {
  ...
  if (s4.has(document.querySelector('.i-4').value)) {
...
const f7 = () => {
  ...
  let s7 = new Set(inp);
  if (inp.length === s7.size && inp.length > 6) {
    out.textContent = 1;
  } else {
    out.textContent = 0;
  }
...
const f10 = (out_set, elem) => {
  document.querySelector(elem).textContent = Array.from(out_set).join(' ');
};
...
const f13 = () => {
  let charCount = {};
  let cleanedStr = str13.replace(/\s/g, '');

  for (let item of cleanedStr) {
