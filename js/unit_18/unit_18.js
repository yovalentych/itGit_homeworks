...
function t4() {
  let spanTask = document.querySelectorAll('.task-4');
  spanTask.forEach((elem) => {
    let dataValue = elem.getAttribute('data');
    a4_res.push(dataValue);
  });
...
function t5(event) {
  a5_res.push(event.target.getAttribute('data'));
...
function t9() {
  a9.forEach((elem) => (a9_res += `${elem.join('-')}-`));
