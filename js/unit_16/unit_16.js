...
function f3() {
  let output = document.getElementsByClassName('out-3');
  for (i of output) {
    i.textContent = 3;
  }
}
...
function f4() {
  let output = document.querySelectorAll('.out-4');
  for (i of output) {
    i.textContent = 4;
  }
}
...
function f10() {
  let obj = {};
  for (item in a10) {
    if (typeof a10[item] == 'string') {
      obj[a10[item].toLowerCase()] = a10[item].toLowerCase();
    } else {
      obj[a10[item]] = a10[item];
    }
  }
  return obj;
}
...
function f15() {
  let outputList = document.querySelectorAll('.out-15');
  for (item of outputList) {
    item.textContent = 15;
  }
}
