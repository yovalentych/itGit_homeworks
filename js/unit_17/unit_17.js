...
function t8() {
  ...
  return a8_res.filter((index) => index !== undefined);
}
...
function t14() {
  a14.forEach((elem, ind, arr) => {
    arr[ind] = elem.toLowerCase();
  });
...
function t15() {
  a15_res = a15.filter((value) => {
    return value.pnum.length === 6;
  });
  return a15_res;
}
